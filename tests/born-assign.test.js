...
import { bornAssign } from '../src/core/index.js';

// ── the regression lock for exp-0005: bornAssign, sign-aware Born assignment ──
//
// bornAssign turns a direction into the reading it belongs to. The capability:
...
test('squared assignment is sign-blind: ±pole of one lens are the SAME reading', () => {
  ...
  assert.equal(bornAssign(plus, lens), 0);
  assert.equal(bornAssign(minus, lens), 0, 'squared cannot tell +axis from -axis');
});

test('signed assignment separates the ± poles of a bipolar lens', () => {
  const lens = [{ lens: e(4, 0) }, { lens: e(4, 1) }];
  const plus = e(4, 0), minus = e(4, 0).map((x) => -x);
  assert.equal(bornAssign(plus, lens, { signed: true }), 0, '+pole of lens 0');
  assert.equal(bornAssign(minus, lens, { signed: true }), 1, '-pole of lens 0 → different reading');
  // a second lens's +pole is a distinct reading again
  assert.equal(bornAssign(e(4, 1), lens, { signed: true }), 2);
});
...
test('signed resolves a balanced two-cluster split the squared rule collapses', () => {
  ...
  const sq = [...left, ...right].map((u) => bornAssign(u, lens));
  const sg = [...left, ...right].map((u) => bornAssign(u, lens, { signed: true }));
...
test('bornAssign accepts bare lens vectors and is pure', () => {
  const lenses = [e(3, 0), e(3, 1), e(3, 2)];   // bare vectors, not {lens}
  const u = [0.1, 0.9, 0.2], before = u.slice();
  assert.equal(bornAssign(u, lenses), 1);
  assert.deepEqual(u, before, 'input not mutated');
  assert.equal(bornAssign(u, lenses), bornAssign(u.slice(), lenses), 'deterministic');
});
