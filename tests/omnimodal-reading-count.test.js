...
import { buildDensity, eigenLenses, readingCount } from '../src/core/index.js';

// ── the regression lock for exp-0003: the geography-derived reading count ─────
//
// readingCount is the omnimodal sense's answer to "how many readings does this
// field hold?" — not a universal constant but a property of the spectrum's shape,
...
test('readingCount finds the elbow k on a spectrum with a real gap', () => {
  ...
  for (const K of [2, 3, 5]) {
    const rc = readingCount(withGap(K));
  ...
  assert.equal(readingCount(withGap(1)).abstain, false);
});

test('readingCount finds real structure through the engine (not flat)', () => {
  // three jittered clusters → a genuine elbow: the count is signal-set (≥2), not a cap
  const rc = readingCount(spectrum(kReadingStream(24, 3, 30, 13)));
...
test('readingCount abstains (k=1) on a flat / isotropic spectrum', () => {
...
test('readingCount is pure: no mutation, deterministic, cap respected', () => {
  ...
  assert.ok(readingCount(many, { maxK: 4 }).k <= 4, 'maxK caps the count');
});

test('readingCount is safe on degenerate inputs', () => {
  assert.deepEqual(readingCount([]), { k: 0, gap: 0, floor: null, abstain: true });
  assert.equal(readingCount([0.5]).k, 1);
  assert.equal(readingCount([Infinity, NaN, 0.3]).k, 1);   // non-finite filtered, one left
});
