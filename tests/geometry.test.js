...
test('the cognition triad is reader · surfer · talker, the surfer in the middle', () => {
  assert.deepEqual(COGNITION_ORDER, ['reader', 'surfer', 'talker']);
  ...
  assert.equal(COGNITION.talker.position, 'last');
  ...
  assert.deepEqual(COGNITION.talker.operators, ['DEF', 'EVA', 'REC']);   // Interpretation
  assert.deepEqual(COGNITION.talker.gate, ['DEF', 'EVA', 'REC'], 'the talker gates with DEF·EVA·REC');
});
...
test('an operator names the faculty that fired it (by Domain)', () => {
  ...
  assert.equal(facultyOfOperator('REC'), 'talker');
