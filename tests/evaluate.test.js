...
// ── the regression lock for exp-0008: evaluate (EVA), reinforce/strain/defeat ──
//
// evaluate folds a unit's fit into a reading's {support, strain} ledger and defeats the
...
const fold = (fits, opts) => { let led = { support: 0, strain: 0 }, at = -1; fits.forEach((f, i) => { led = evaluate(led, f, opts); if (led.defeated && at < 0) at = i; }); return { led, at }; };
...
test('evaluate is a pure fold step (no mutation, deterministic)', () => {
  const led = { support: 1, strain: 0.2 }, before = { ...led };
  const a = evaluate(led, 0.5), b = evaluate({ ...led }, 0.5);
  assert.deepEqual(led, before, 'input ledger not mutated');
  assert.deepEqual(a, b, 'same inputs → same result');
  assert.deepEqual(evaluate(undefined, 0.5), evaluate({ support: 0, strain: 0 }, 0.5), 'missing ledger = 0,0');
});
