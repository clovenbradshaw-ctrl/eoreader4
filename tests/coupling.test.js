...
import { coupling } from '../src/core/index.js';

// ── the regression lock for exp-0006: coupling, the two-way holon atom ────────
//
// coupling(part, whole) decomposes part = k·whole + residual and returns the pull
...
test('a part fully set by the whole has pull 1 and ~zero residual', () => {
  ...
  const { pull, k, residual } = coupling(part, whole);
...
test('coupling is safe on a null whole and pure on its inputs', () => {
  const part = [1, 2, 3], zero = [0, 0, 0], before = part.slice();
  const { pull, residual } = coupling(part, zero);
  assert.equal(pull, 0);
  assert.deepEqual(residual, part, 'no whole → residual is the part');
  assert.deepEqual(part, before, 'inputs not mutated');
  assert.deepEqual(coupling(part, [1, 1, 1]), coupling(part.slice(), [1, 1, 1]), 'deterministic');
});
