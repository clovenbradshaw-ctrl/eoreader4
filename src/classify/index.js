// The classify holon: the geometric reader — centroid measurement of a
// proposition into its three-position phasepost (Ground, Figure, Pattern).
//
// Classification is measurement, not choice. The cell a proposition fills is
// read off its position in centroid space, not chosen by a reader. This is the
// difference between the control-flow router (a reader may emit a route) and
// the classifier (it measures an address). The firewall: under the hash
// embedder the classifier returns no-commit on every position, because a cosine
// between a spelling-space query and a MiniLM-space centroid measures nothing.

export { createPhasepostClassifier, perceptionDeposit, DEFAULT_FLOORS } from './phasepost.js';
export { BANDS, BAND_OPERATORS, bandOf, partitionCells, isDesert, isMisfireCell } from './bands.js';
export { loadCentroids, CENTROID_SCHEMA } from './centroids.js';
