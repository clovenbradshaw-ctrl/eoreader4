...
class Component extends DCLogic {
  ...
  _headCandidate(s){s=this.norm(s);if(s.length>64||s.length<1)return null;
    const kw=/^(chapter|chap\.?|part|book|canto|letter|act|scene|volume|stave|section|prologue|epilogue|introduction|preface|argument|chapitre|capitulo|capítulo|kapitel|глава)\b/i.test(s);
    const rk=s.match(/(?:^|\s)([IVXLCDM]{1,7})\.?$/),ak=s.match(/(?:^|\s)(\d{1,3})\.?$/);
    if(kw)return {label:s,kind:'kw',num:rk?this._roman(rk[1]):(ak?+ak[1]:null)};
  ...
  detectStructure(p,paras){
    ...
    const keep=new Set();
    cands.forEach(c=>{if(c.kind==='kw')keep.add(c.i);});
    ...
    if(accepted.length>=2)return accepted.map(i=>({paraIndex:i,label:this.norm(paras[i]).slice(0,72),kind:'heading'}));
    ...
    const peaks=[];for(let i=1;i<N;i++){const L=win(i-W,i),R=win(i,i+W);if(L.size<2||R.size<2)continue;let x=0;for(const e of L)if(R.has(e))x++;const d=1-x/Math.sqrt(L.size*R.size);if(d>0.6)peaks.push({i,d});}
    ...
    for(const i of chosen){let anchor=i,label;
      ...
      if(seen.has(anchor))continue;seen.add(anchor);out.push({paraIndex:anchor,label,kind:'emergent'});}
    return out;}
  ...
  _bookHtml(p){
    ...
    paras.forEach((t,i)=>{
      ...
      if(hit){const id='eo-ch-'+hit.n;toc.push({id,label:this.norm(hit.s.label)});
        if(hit.s.kind==='heading'||this._titleish(t)){parts.push('<h2 class="eo-chap'+(hit.s.kind==='emergent'?' eo-emergent':'')+'" id="'+id+'">'+esc(t)+'</h2>');return;}
