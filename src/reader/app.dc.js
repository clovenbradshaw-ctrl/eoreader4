...
class Component extends DCLogic {
  ...
  // ── document structure: best-guess TOC via the engine's own reading ──────
  // Headings alone are unreliable (see docs/structure.md): absent in many books,
  // multilingual, and bare numerals collide with page numbers. So structure is GUESSED
  // from two signals the engine arbitrates between:
  //   1. Heading candidates — explicit markers / numerals / all-caps lines — kept only
  //      when a keyword OR part of a consecutive run (I,II,III… / 1,2,3…). The run test
  //      recovers a missed first member and rejects strays (42, MIX, a dateline).
  //   2. The entity field — the entities the engine projects per paragraph. A real
  //      boundary is where that field SHIFTS and the new field PERSISTS across a window
  //      (a one-line blip can't fake it; both sides need real entity mass).
  // Explicit runs win when present; otherwise sections EMERGE from field shifts — the
  // category is the output of the reading, not its input. The same boundaries let the
  // reading cursor move by structural unit.
  _roman(r){const m={I:1,V:5,X:10,L:50,C:100,D:500,M:1000};let n=0;r=String(r).toUpperCase();for(let i=0;i<r.length;i++){const a=m[r[i]],b=m[r[i+1]];if(!a)return null;if(b&&a<b){n+=b-a;i++;}else n+=a;}return n;}
  _headCandidate(s){s=this.norm(s);if(s.length<1||s.length>200)return null;
    // Markdown ATX heading (#..######) — the reader imports .md; level = depth.
    const md=s.match(/^(#{1,6})\s+(.+?)\s*#*$/);if(md)return {label:md[2],kind:'md',num:null,level:md[1].length};
    if(s.length>64)return null;
    // Decimal section number (1.2, 3.3.2) — technical docs; level = depth.
    const dec=s.match(/^(\d+(?:\.\d+)+)\.?(?:\s|$)/);if(dec)return {label:s,kind:'dec',num:null,level:dec[1].split('.').length};
    const words=s.split(/\s+/).length;
    const kwStart=/^(chapter|chap\.?|part|book|canto|letter|act|scene|volume|stave|section|prologue|epilogue|introduction|preface|argument|chapitre|capitulo|capítulo|kapitel|глава)\b/i.test(s);
    // Keyword after a short Capitalized prefix, ending at the numeral ("Inferno: Canto I").
    // Anchored + ends-at-numeral so it can't catch list captions ("Heading to Chapter I. 1").
    const kwAnyRe=/^(?:[A-Z][^\s]*\.?\s+){0,2}(chapter|chap|part|book|canto|act|scene|volume|stave|letter|section)\s+([ivxlcdm]+|\d+)\.?$/i;
    const m2=words<=6?s.match(kwAnyRe):null;
    const rk=s.match(/(?:^|\s)([IVXLCDM]{1,7})\.?$/),ak=s.match(/(?:^|\s)(\d{1,3})\.?$/);
    if(kwStart||m2){const nm=(m2&&m2[2])||(rk&&rk[1])||(ak&&ak[1]);return {label:s,kind:'kw',num:nm?(/^\d+$/.test(nm)?+nm:this._roman(nm)):null};}
    if(/^[IVXLCDM]{1,7}\.?$/.test(s))return {label:s,kind:'roman',num:this._roman(s.replace(/\.$/,''))};
    if(/^\d{1,3}\.?$/.test(s))return {label:s,kind:'num',num:+s.replace(/\.$/,'')};
    if(s.length<=44&&/[A-Z]{3,}/.test(s)&&!/[a-z]/.test(s)&&/^[A-Z0-9][A-Z0-9 ,.'’\-—:;!?()&]+$/.test(s)&&!/[.!?]$/.test(s))return {label:s,kind:'caps',num:null};
    return null;}
  ...
  _paraField(p,paras){
    ...
    return sets;}
  // → [{paraIndex,label,kind:'heading'|'emergent'}] in reading order.
  detectStructure(p,paras){
    const sets=this._paraField(p,paras);
    const cands=paras.map((t,i)=>{const c=this._headCandidate(t);return c?Object.assign(c,{i}):null;}).filter(Boolean);
    const keep=new Set(),lvl={};
    // Explicit markers — keyword, markdown, decimal — are kept outright (with their level).
    cands.forEach(c=>{if(c.kind==='kw'||c.kind==='md'||c.kind==='dec'){keep.add(c.i);if(c.level)lvl[c.i]=c.level;}});
    // Bare numerals only when part of a consecutive run (recovers a hidden first member,
    // rejects strays like a lone page number).
    for(const kind of ['roman','num']){const seq=cands.filter(c=>c.kind===kind&&c.num!=null);
      for(let a=0;a<seq.length;a++){const run=[seq[a]];for(let b=a+1;b<seq.length;b++){if(seq[b].num===run[run.length-1].num+1)run.push(seq[b]);else break;}
        if(run.length>=2){run.forEach(c=>keep.add(c.i));a+=run.length-1;}}}
    const accepted=[...keep].sort((a,b)=>a-b);
    if(accepted.length>=2)return accepted.map(i=>({paraIndex:i,label:this.norm(paras[i]).slice(0,72),kind:'heading',level:lvl[i]||1}));
    // Emergent: window comparison so a single blip can't fake a boundary; both sides
    // need ≥2 distinct entities (an essay with no named entities yields nothing).
    const N=paras.length,W=Math.max(2,Math.round(N/24));
    ...
    // Contrast guard: if almost every gap is a "boundary", the field is uniformly choppy —
    // a glossary or anthology of independent short entries, not a chaptered work. Structure
    // needs a stable background to stand out against (the repo's null-boundary idea); with
    // none, refuse to invent one. (Better no TOC than a hallucinated one.)
    if(valid>=8&&deep/valid>0.5)return [];
    ...
    for(const i of chosen){let anchor=i,label;
      if(i>0&&this._titleish(paras[i-1])&&!this._headCandidate(paras[i-1])){anchor=i-1;label=this.norm(paras[i-1]).slice(0,72);}
