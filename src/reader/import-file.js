...
// (whisper, pdf.js, Tesseract, SheetJS, Readability) are the same "inject the library,
...
export async function importAnyFile(file, opts = {}) {
  ...
  // Scanned image — Tesseract word boxes (ocr organ).
...
async function fromImage(file, title, name, say) {
  ...
  try {
    ...
    if (!doc.text || !doc.text.trim()) throw new Error('no text found in the image');
    return { text: doc.text, title, meta: { modality: 'ocr', doc } };
