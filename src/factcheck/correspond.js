...
const voidDenial = async ({ src, tgt, talkerCell }, { graph, classifier, adj }) => {
  ...
  for (const v of voids) {
    if (v.kind === 'same_as?' || v.kind === 'same-proposition?') continue;
                                                    // an IDENTITY void (entity same_as? or the proposition
                                                    // asterisk) is not a fact-relation void — it holds open who
                                                    // two names / two assertions are, not what a relation denies
                                                    // (asterisk.js, proposition-equivalence.js)
